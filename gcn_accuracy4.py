...
class GCNSampling1(nn.Module):
    def __init__(self,
                 ...
                 activation,
                 dropout):
        super(GCNSampling1, self).__init__()
    ...
    def forward(self, nf):
        ...
        for i, layer in enumerate(self.layers):
            if i == 0:
                ...
                nf.apply_layer(0, layer)  # layer0 降维
...
fc = nn.Linear(in_features=500, out_features=16, bias=True)
...
# '''training'''
model1 = GCNSampling1(in_feats, n_hidden, n_classes, n_layers, F.relu, dropout)
...
for epoch in range(1):
    for nf in dgl.contrib.sampling.NeighborSampler(new_g, batch_size,
        ...
        for index8 in range(len(sum_nf)):
            if sum_nf[index8] not in (sub_node_id[0]):  # 不是自己方
                for n2 in range(len(send_index1)):
                    map_g = new_g.parent_nid[sum_nf[index8]]  # 在g中的ID
                    if map_g in sub_node_list[send_index1[n2]]:  # send_index1[n2] 方
                        fc.weight = nn.Parameter(Model[send_index1[in2]]['layers.0.linear.weight'])
                        fc.bias = nn.Parameter(Model[send_index1[in2]]['layers.0.linear.bias'])
                        new_g.nodes[sum_nf[index8]].data['activation'] = \
